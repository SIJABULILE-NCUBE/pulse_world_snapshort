...
For the graded submission requirements (schema, screenshots, paragraph, checklist), see [`SUBMISSION.md`](./SUBMISSION.md) in this repository. That file contains everything needed for marking in one place.
...
1. **Data collection.** `fetch.js` communicates with the external APIs and writes the results into Supabase.
2. **Data presentation.** The files inside `public/` communicate with Supabase and display the stored data to the user.
...
```
git clone https://github.com/SIJABULILE-NCUBE/pulse_world_snapshort.git
cd pulse_world_snapshort
```
...
Create a Supabase project at [supabase.com](https://supabase.com). Inside Supabase:
...
Repository: [https://github.com/SIJABULILE-NCUBE/pulse_world_snapshort](https://github.com/SIJABULILE-NCUBE/pulse_world_snapshort)
